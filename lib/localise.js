...
module.exports = function(debug, i18next, options) { 
	// Convert `path` option into a function if a String has been passed
  var fnFromString = (value) => typeof value === 'string' ? () => value : value;
  var getPath = fnFromString(options.path);

	var localisedFilePath = helpers(i18next, {path: getPath}).localisedFilePath,
